...
InMemoryStorage를 PostgresStorage로 교체. 주문과 FSM 전이가 DB에 영속된다.

## 2. 합격 기준 매핑

- Phase 1 합격기준: 3
- 기여 방식: 일일 P&L 자동 기록 — DB 없으면 기록 불가
...
1. `docs/specs/db-schema-phase1.sql` — 전체 (6테이블 DDL) — 테이블 구조와 인덱스 (20분)
2. `docs/decisions/006-db-stack.md` — 전체 — PostgreSQL 선정 근거와 설정 (10분)
