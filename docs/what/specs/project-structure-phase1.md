# Phase 1 프로젝트 폴더 구조 (Hexagonal 3층·의존방향)
...
1. **설계 문서 구조와 1:1 미러링** — 설계(docs/what/specs/port-signatures)에 정의된 Port가 코드에서도 동일한 경로(`ports/`)에 위치.
...
│   │   │
│   │   ├── nodes/                        ← Path 1 노드 7개 (ADR-013)
│   │   │   ├── __init__.py
│   │   │   ├── market_data_receiver.py   ← MarketDataReceiver
│   │   │   ├── indicator_calculator.py   ← IndicatorCalculator
│   │   │   ├── strategy_engine.py        ← StrategyEngine
│   │   │   ├── risk_guard.py             ← RiskGuard (Pre-Order 8체크)
│   │   │   ├── order_executor.py         ← OrderExecutor (요청만, 축소)
│   │   │   ├── execution_receiver.py     ← ExecutionReceiver (신설, ADR-013)
...
│   │
│   ├── ports/                            ← Port ABC 7개 (port-signatures-phase1.md 그대로)
│   │   ├── __init__.py
│   │   ├── exceptions.py                 ← PortError 계층
│   │   ├── market_data_port.py
│   │   ├── order_port.py                 ← OrderPort (BrokerPort 분리)
│   │   ├── account_port.py               ← AccountPort (BrokerPort 분리)
│   │   ├── storage_port.py
│   │   ├── clock_port.py
│   │   ├── strategy_runtime_port.py
│   │   ├── audit_port.py
│   │   └── execution_event_port.py          ← ExecutionEventPort ABC (ADR-013)
│   │
│   ├── adapters/                         ← 구현체 19개 (adapter-spec-phase1.md 그대로)
│   │   ├── __init__.py
│   │   ├── factory.py                    ← order/account/execution_event/market_data 모드 기반 팩토리
│   │   │
│   │   ├── market_data/
│   │   │   ├── __init__.py
│   │   │   ├── kis_websocket.py
...
│   │   │
│   │   ├── order/                        ← OrderPort 어댑터 (BrokerPort 분리)
│   │   │   ├── __init__.py
│   │   │   ├── mock_order.py
│   │   │   ├── kis_paper_order.py
│   │   │   └── synthetic_order.py
│   │   │
│   │   ├── account/                      ← AccountPort 어댑터 (BrokerPort 분리)
│   │   │   ├── __init__.py
│   │   │   ├── mock_account.py           ← MockOrder와 in-process 상태 공유
│   │   │   ├── kis_paper_account.py
│   │   │   └── synthetic_account.py      ← ExchangeEngine 공유
│   │   │
│   │   ├── execution_event/              ← ExecutionEventPort 어댑터 (ADR-013 신설)
│   │   │   ├── __init__.py
│   │   │   ├── mock_event_bus.py         ← MockEventBus (MockOrder와 공유)
│   │   │   ├── mock_execution_event.py   ← MockExecutionEventAdapter
│   │   │   ├── kis_paper_execution_event.py  ← KIS H0STCNI0 WebSocket
│   │   │   └── synthetic_execution_event.py  ← ExchangeEngine 체결 콜백
│   │   │
│   │   ├── exchange/                     ← 가상거래소 (synthetic-exchange-phase1.md)
│   │   │   ├── __init__.py
│   │   │   ├── engine.py                 ← ExchangeEngine 핵심 코어
│   │   │   ├── price_generator.py        ← GBM Level 1/2/3
│   │   │   ├── order_book.py
│   │   │   ├── market_rules.py
...
│   └── init.sql                          ← docs/what/specs/db-schema-phase1.sql 복사본
│
...
│   │   │   └── fsm/
│   │   ├── adapters/
│   │   │   ├── order/                    ← MockOrder 멱등성, 슬리피지
│   │   │   ├── account/                  ← MockAccount 잔고/포지션
│   │   │   ├── exchange/                 ← ExchangeEngine, OrderBook, MarketRules
│   │   │   ├── market_data/
│   │   │   └── storage/
│   │   └── infrastructure/
│   │
...
| Port ABC | `XxxPort` | `OrderPort`, `AccountPort` |
| Adapter | `XxxAdapter` | `KISPaperOrderAdapter`, `MockAccountAdapter` |
...
Step 7: Real Adapters (2~3일)
  - KISWebSocketAdapter, KISRestAdapter, KISPaperOrderAdapter, KISPaperAccountAdapter
