...
**상태**: Accepted
**작성일**: 2026-04-17
**관련**: ADR-011 (Phase 1 Scope), `docs/what/architecture/path1-phase1.md`, `docs/what/specs/project-structure-phase1.md`
...
What 층 (기존 설계)  — 시스템이 무엇인가. SSoT. 불변 원칙.
  ├── ADR-011, path1-phase1.md, port-signatures-phase1.md, domain-types 등
  └── 5차 검증 188항목 정합성 보존
...
|------|-------|---------|--------|---------|------|
| 0 | Walking Skeleton 단일 파일 | 파이프라인 관통 | (기반) | `path1-phase1.md §2`, `domain-types-phase1.md §3.1~3.5`, `system-overview.md` | 1일 |
...
| 4 | IndicatorCalculator 실제 | 실제 지표 값 | 1 | `path1-phase1.md §2.2` | 1일 |
| 5 | StrategyEngine 실제 | 조건부 신호 | 1 | `path1-phase1.md §2.3`, `domain-types-phase1.md §3.4` | 1일 |
| 6 | RiskGuard 포지션 한도 | 첫 거부 가능 | 2 | `path1-phase1.md §2.4,§5`, `error-handling-phase1.md §3.4` | 1일 |
| 7 | MockBrokerAdapter + OrderExecutor | 체결 응답 수신 | 2 | `adapter-spec-phase1.md §5.1`, `path1-phase1.md §2.5`, `error-handling-phase1.md §3.5` | 1일 |
| 8a | 개별 종목 FSM 기본 4상태 (IDLE/ORDER_PLACED/HOLDING/DONE) | 기본 전이 | 2 | `fsm-design.md §3` (IDLE~HOLDING~DONE 경로만) | 1일 |
| 8b | 개별 종목 FSM 나머지 (13상태 완전) | 전체 전이 | 2, 5 | `fsm-design.md` 전체, `boot-shutdown-phase1.md §3` | 1일 |
| 9 | DB 영속화 | 주문 DB 기록 | 3 | `db-schema-phase1.sql`, `006-db-stack.md`, `db-stack.md` | 2일 |
| 10a | RiskGuard 손실한도 | 손실 거부 | 2 | `path1-phase1.md §5`, `config-schema-phase1.md §5` | 1일 |
| 10b | RiskGuard 변동성 | 변동성 거부 | 2 | `path1-phase1.md §5` | 1일 |
| 10c | CLI start/stop/status | CLI 제어 | (기반) | `cli-design.md §2~5`, `boot-shutdown-phase1.md §2,§4` | 1일 |
| 10d | CLI halt 30초 | halt 동작 | 4 | `cli-design.md §5.2`, `boot-shutdown-phase1.md §5`, `fsm-design.md §3` (SUSPENDED) | 1일 |
