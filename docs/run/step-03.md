...
> **대안 경로**: CSV 데이터가 없는 경우 `SyntheticMarketAdapter + SyntheticOrderAdapter + SyntheticAccountAdapter`로도 이 Step을 완주할 수 있다.
> `config.yaml`에서 `market_data.mode: synthetic` + `order.mode: synthetic` + `account.mode: synthetic` 으로 전환하면 된다.
...
1. `docs/what/pipelines/data-collection.md` — §1 정형 데이터 수집 파이프라인 — CSV 파일 포맷·컬럼 확인 (10분)
2. `docs/what/specs/adapter-spec-phase1.md` — §4.3 CSVReplayAdapter — Adapter 구현 명세 (5분)
3. `docs/what/specs/domain-types-phase1.md` — §3.3 Market Data (Bar 타입) — 필드 확인 (3분)
